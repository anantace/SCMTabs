...
<table class="default scm nohover">
    ...
    <thead class="content_title">
        <tr>
            ...
            <td>
            <? if ($scm->user): ?>
                <?= sprintf(_('Zuletzt geädert von %s am %s'),
...
<? if ($priviledged): ?>
    <tfoot class="table_footer">
        <tr>
            <td colspan="2">
                <?= LinkButton::create(_('Bearbeiten'), PluginEngine::getLink($this->plugin, array(), 'show/edit/' . $scm->id), array('data-dialog' => '')) ?>
        <? if (count($scms) > 1): ?>
            <? if ($scm->position == 0): ?>
            ...
            <? endif; ?>
                <?= LinkButton::create(_('Löschen'),
                                       $controller->url_for('show/' . $scm->id . '?verify=delete'),
