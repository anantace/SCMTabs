 ...
 * @author   Annelene Sudau <asudau@uni-osnabrueck.de>
...
class ShowController extends StudipController
{
    ...
    public function index_action($id = null)
    {
        ...
        $temp       = SCMTabEntry::findByRange_id($GLOBALS['SessSemName'][1], 'ORDER BY position ASC');
        ...
        if (Request::get('verify') == 'delete') {
            $this->verification = $GLOBALS['template_factory']->open('shared/question')->render(array(
                'approvalLink'    => $this->url_for('show/delete/' . $this->scm->id . '?ticket=' . get_ticket()),
                'disapprovalLink' => $this->url_for('show/' . $this->scm->id),
                'question'        => _('Wollen Sie diese Seite wirklich lˆschen?'),
            ));
        }

        $this->set_title($this->scm->tab_name);
        Navigation::activateItem('/course/scmTabs' . $this->scm->id);
    }
    ...
    public function edit_action($id = null)
    {
        if (Request::submitted('submit')) {
            ...
            if ($scm->store() !== false) {
                $message = MessageBox::success(_('Die ƒnderungen wurden ¸bernommen.'));
                PageLayout::postMessage($message);
            }

            $this->redirect('show/' . $scm->id);
        }

        $this->scm = new ScmTabEntry($id);
    ...
    public function delete_action($id)
    {
        ...
        if ($ticket && check_ticket($ticket)) {
            ...
            if (!$scm->isNew() && $scm->range_id == $GLOBALS['SessSemName'][1]){
                $scm->delete();
                PageLayout::postMessage(MessageBox::success(_('Der Eintrag wurde gelˆscht.')));
            }
            $this->redirect('show/');
            return;
        }

        PageLayout::postMessage(MessageBox::error(_('Es ist ein Fehler aufgetreten. Bitte versuchen Sie erneut, diese Seite zu lˆschen.')));
        $this->redirect('show/' . $id);
    }
